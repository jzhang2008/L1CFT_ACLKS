function results = L1CFT_ACLKS_HC(params)
%% Initialization
...
[currentScaleFactor, base_target_sz, ~, img_sample_sz, use_sz] = init_size(params,target_sz,search_area);
...
%     reg_window = 1-exp(-(abs(wrs/reg_scale(1)).^params.reg_window_power + abs(wcs/reg_scale(2)).^params.reg_window_power)/12);
% end
% Define spatial regularization windows
if params.use_reg_window
    reg_scale = floor(1.0*base_target_sz/featureRatio);    
...
[ysf, scale_window, scaleFactors, scale_model_sz, min_scale_factor, max_scale_factor] = init_scale(params,target_sz,img_sample_sz,base_target_sz,im);
% % scale factors
% if nScales > 0
...
buffer_st_xl   = cell(1,params.L);
buffer_gf      = cell(1,params.L);
buffer_idx     =0;
% initialize SESF 
Dx_pred = 0;
Dy_pred = 0;
...
%             else
%                 scores_fs_padded   = responsef_padded{1};
%                 scores_fs          = response{1};
%                 scale_change_factor=scaleFactors(sind{1});
%             end
            % find maximum
            [disp_row, disp_col, sind] = resp_newton(response{1}, responsef_padded{1}, newton_iterations, ky, kx, use_sz);
%             scale_change_factor= scaleFactors(sind);
            scores_fs          = response{1}(:,:,sind);
            % Compute the translation vector in pixel-coordinates and round
            % to the closest integer pixel.
            translation_vec = [disp_row, disp_col] * featureRatio * currentScaleFactor;
            % update position
            old_pos = pos;
...
%         end
        % generate the mask which can subtract the object region
        [Nonobj_m,im_c] = NonObject_mask(im,pos,target_sz);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Model update step
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         im_c = im.*Nonobj_m;
        if params.distractor_num>0
           for p=1:params.distractor_num
            % extract features
              xl_context{p} = extract_features(im_c, local_pos(p,:), currentScaleFactor, features, global_fparams, feature_extract_info);
            % do windowing of features
...
%        g_pre_f_new =cellfun(@(pre_f,f) (1-params.learning_rate)*pre_f+params.learning_rate*f, g_pre_f_new,buffer_gf{corr_idx}, 'uniformoutput', false);
       if length(corrValue)>=params.L
...
%     end
    ...
    if seq.frame==1 
        ...
        model_xlf{1} = (1-params.learning_rate)*model_xlf{1}+params.learning_rate*xlf{1};
    end
    [cf_f,g_pre_f]=train_filter(yf,model_xlf,xlf_context,cf_f,g_pre_f_new,params,seq,use_sz,small_filter_sz);
%     [cf_f{1}] = training(model_xlf{1}, xlf{1}, xlf{1}, use_sz, params, yf, small_filter_sz, seq.frame);
%     g_pre_f = cf_f{1};
    %keyfilter update
