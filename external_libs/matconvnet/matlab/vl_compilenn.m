...
% --------------------------------------------------------------------
mybase  = flags.base;                                                      %添加此代码在于CUDA10.1以上将优化指令修改为了-O+数字
mybase(3)={'-O3'};                                                     
if check_deps(opts, tgt, src), return ; end
...
if status 
    error('Command %s failed.', nvcc_cmd); 
end
...
cl_path = fullfile(cc.Location, 'VC', 'Tools', 'MSVC','14.28.2933','bin','Hostx64','x64');                     %鏍规嵁鑷繁VC鐨勫畨瑁呯幆澧冧慨鏀硅矾寰?
...
for v = {'5.5', '6.0', '6.5', '7.0', '7.5', '8.0', '8.5', '9.0', '9.5', '10.0', '11.0'}
  switch computer('arch')
    case 'glnxa64'
